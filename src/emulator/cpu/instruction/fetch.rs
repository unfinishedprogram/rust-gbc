...
pub fn fetch_instruction<T: CPU + MemoryMapper>(cpu: &mut T) -> Instruction {
	...
	match (x, z, y, p, q) {
		...
		(0, 1, _, _, 0) => inst!(cpu, LD_16, (DT.rp[p].clone()), nn),

		(0, 1, _, _, 1) => inst!(cpu, ADD_16, HL, (DT.rp[p].clone())),
		...
		(0, 3, _, _, 0) => inst!(cpu, INC_16, (DT.rp[p].clone())),
		(0, 3, _, _, 1) => inst!(cpu, DEC_16, (DT.rp[p].clone())),
		...
		(2, _, _, _, _) => inst!(cpu, ALU_OP_8, (DT.alu[y].clone()), A, (DT.r[z].clone())),
		...
		(3, 2, c, _, _) => inst!(cpu, JP, (DT.cc[c].clone()), nn),
		...
		(3, 6, _, _, _) => inst!(cpu, ALU_OP_8, (DT.alu[y].clone()), A, n),
